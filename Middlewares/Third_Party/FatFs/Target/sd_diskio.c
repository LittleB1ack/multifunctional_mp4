...
/* USER CODE BEGIN firstSection */
/* Workaround: put project-specific includes in USER CODE so CubeMX won't
  overwrite them. These are needed for diskio/FatFs integration and for
...
enable the defines below to send custom rtos messages
              SCB_InvalidateDCache_by_Addr((uint32_t*)alignedAddr, count*SD_DEFAULT_BLOCK_SIZE + ((uint32_t)buff - alignedAddr));
...
ALIGN_32BYTES(static uint8_t scratch[SD_DEFAULT_BLOCK_SIZE]); /* 32-Byte aligned for cache maintenance */
#else
__ALIGN_BEGIN static uint8_t scratch[SD_DEFAULT_BLOCK_SIZE] __ALIGN_END;
#endif
#endif
/* Disk status */
...
Stat = STA_NOINIT;
...
#endif
  {
    ...
    {
      printf("[DISKIO] ===== SD INITIALIZE =====\r\n");
      printf("[DISKIO] BSP_SD_Init returned MSD_OK\r\n");
      Stat = SD_CheckStatus(lun);
      printf("[DISKIO] SD_CheckStatus -> Stat=0x%02X\r\n", Stat);
      printf("[DISKIO] -----------------------------\r\n");
    }
...
/**
  * @brief  Reads block(s) from a specified address in an SD card, in polling/DMA mode.
  * @param  lun : not used
